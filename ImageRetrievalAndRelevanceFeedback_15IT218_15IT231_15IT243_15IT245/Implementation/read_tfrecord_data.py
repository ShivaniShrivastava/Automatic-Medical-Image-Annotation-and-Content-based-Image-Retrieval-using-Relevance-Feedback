...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_integer("image_number", 38794, "Number of images in your tfrecord, default is 300.")
flags.DEFINE_integer("class_number", 193, "Number of class in your dataset/label.txt, default is 3.")
flags.DEFINE_integer("image_height", 256, "Height of the output image after crop and resize. Default is 299.")
flags.DEFINE_integer("image_width", 256, "Width of the output image after crop and resize. Default is 299.")
...
with tf.Session() as sess:
    ...
    print("Write cropped and resized image to the folder './class_image'") 
    for i in range(FLAGS.image_number): # number of examples in your tfrecord
        pre_image, pre_label,pre_filename = sess.run([current_image_object.image, current_image_object.label,current_image_object.filename])
        img = Image.fromarray(pre_image, "RGB")
        if not os.path.isdir("./class_image/"):
            os.mkdir("./class_image")
        img.save(os.path.join("./class_image/class_"+str(pre_label)+"_Index_"+str(i)+ "_fname_"+ str(pre_filename)+ ".jpeg"))
        if i % 10 == 0:
...
print("cd to current directory, the folder 'class_image' should contains %d images with %dx%d size." % (FLAGS.image_number,FLAGS.image_height, FLAGS.image_width))
